...
def random_0k(G0, nswap=1, max_tries=100):  # 基于随机断边重连的0阶零模型
    ...
    while swapcount < nswap:
        ...
        u, v = random.choice(tmp_edges)  # 随机选网络中的一条要断开的边
...
def random_1k(G0, nswap=1, max_tries=100):  # 随机断边重连的1阶零模型
    ...
    # key = [node for (node, val) in G.degree()]
    # keys = [int(x) for x in key]
    keys = [node for (node, val) in G.degree()]
    print(keys)
    print(degrees)
...
def random_25k(G0, nswap=1, max_tries=100):
    ...
    l = map(lambda t: (t[1], t[0]), G0.degree())  # (度,节点)组成的列表
    D = dict_degree_nodeslist(l)
    n = 0
    swapcount = 0
    while swapcount < nswap:
        (u, x) = random.sample(G.nodes(), 2)  # 随机找两个不相连的点
        if (G.has_edge(u, x)): continue
        v = random.choice(list(G[u]))
        y = random.choice(list(G[x]))
        if (G.has_edge(u, x)): print("存在:" + u + "与" + x)
        if G.degree(v) != G.degree(y) or v == y:
            continue  # 若节点v,y度不相同，重新选择
        if (y not in G.neighbors(u)) and (v not in G.neighbors(x)) and ((u, v) in G.edges()) and ((x, y) in G.edges()):
            G.add_edge(u, y)
        ...
        for i in range(len(D)):
            avcG0 = nx.average_clustering(G0, nodes=list(D[i + 1]), weight=None, count_zeros=True)
            avcG = nx.average_clustering(G, nodes=list(D[i + 1]), weight=None, count_zeros=True)
    ...
    while swapcount < nswap:
        ...
        if (y not in G.neighbors(u)) and (v not in G.neighbors(x)) and ((u, v) in G.edges()) and ((x, y) in G.edges()):
            ...
            if not nx.is_connected(G):
                ...
                swapcount -= 1
                continue
            l = map(lambda t: (t[1], t[0]),
                    G0.degree([u, v, x, y] + list(G[u]) + list(G[v]) + list(G[x]) + list(G[y])).items())  # (度,节点)组成的列表
            D = dict_degree_nodeslist(l)
            for i in range(len(D)):
                avcG0 = nx.average_clustering(G0, nodes=D.values()[i], weight=None, count_zeros=True)
                avcG = nx.average_clustering(G, nodes=D.values()[i], weight=None, count_zeros=True)
...
def rich_club_creatc(G0, k, max_tries=100):
    ...
    while len(hubs_edges) < len_possible_edges:
        ...
        if G.degree()[v] > k or G.degree()[x] > k:  # 另一端节点为非富节点
            continue
        if ((x, v) not in G.edges()) and ((v, x) not in G.edges()) and ((u, y) not in G.edges()) and (
                (y, u) not in G.edges()):
            G.add_edge(u, y)
            G.add_edge(x, v)
            G.remove_edge(u, v)
